...
test('micromark-extension-lemmy-spoiler (syntax, container)', async function (t) {
  await t.test('should support a spoiler', async function () {
    assert.equal(micromark(':::spoiler hi', options()), '<details></details>')
  })
  ...
  await t.test('should support a spoiler with many spaces', async function () {
    assert.equal(
      micromark(':::    spoiler hi', options()),
      '<details></details>'
    )
  })

  await t.test('should not support without immediate name', async function () {
    assert.equal(
      micromark(':::spoiler\n hi', options()),
      '<p>:::spoiler\nhi</p>'
    )
  ...
  await t.test(
    'should not support with name on separate line',
    async function () {
      assert.equal(
        micromark(':::spoiler\nhi', options()),
        '<p>:::spoiler\nhi</p>'
  ...
  await t.test('should support any characters in name', async function () {
    assert.equal(
      micromark(':::spoiler -#Hi#$(*@)_', options()),
      '<details></details>'
    )
  })

  await t.test('should not support without name', async function () {
    assert.equal(
      micromark(':::spoiler \nhi', options()),
      '<p>:::spoiler\nhi</p>'
    )
  ...
  await t.test('should not support a partial spoiler name', async function () {
    assert.equal(micromark(':::spoi hi', options()), '<p>:::spoi hi</p>')
  })

  await t.test('should not support too long spoiler name', async function () {
    assert.equal(
      micromark(':::spoilerr hi', options()),
      '<p>:::spoilerr hi</p>'
    )
  })
  ...
  await t.test(
    ...
    async function () {
      assert.equal(micromark(':::9 hi', options()), '<p>:::9 hi</p>')
    }
  )
  ...
  await t.test(
    ...
    async function () {
      assert.equal(micromark(':::- hi', options()), '<p>:::- hi</p>')
    }
  )
  ...
  await t.test('should support no closing fence', async function () {
    assert.equal(micromark(':::spoiler hi\n', options()), '<details></details>')
  })
